...
public class Main {
    private static String moreThan2(Studio[] studioDataBase)
    {
        String s=" ";
        for(int i=0;i<studioDataBase.length;i++){
            if(studioDataBase[i].getContor()>2)
                s+=studioDataBase[i].getNume()+" ";
        }
        return s;
    }

    private static String cautActor(Studio[] studioDataBase,String name)
    {
        String s="";
        for(int i=0;i<studioDataBase.length;i++)
        {
            for(int j=0;j<studioDataBase[i].filme.length;j++)
            {
                ...
                {
                    if(studioDataBase[i].filme[j].actori[k].equals(name))
                        s+=studioDataBase[i].getNume()+" ";
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        Film film4 = new Film(2018,"film cu actori de oscar",new Actor[]{actorOscar2010,actorFaraPremii02,actorOscar2018});
        ...
        System.out.print(moreThan2(StudioDataBase));
        System.out.println(cautActor(StudioDataBase,"actor cu 2 oscaruri "));
    }
}

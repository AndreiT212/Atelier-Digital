...
public class Main
{
    ...
    {
        words=words.stream().filter(word->word.length()%2==0).collect(Collectors.toList());
    ...
    {
        String s=animals.entrySet().stream().map(set->set.getKey()+set.getValue()).collect(Collectors.joining(","));
        System.out.println("["+s+"]");
    }

    public static void main(String[] args)
    {
        List<String> words= Arrays.asList("Apple","Dog","chair","NO");
        ex3(words);
